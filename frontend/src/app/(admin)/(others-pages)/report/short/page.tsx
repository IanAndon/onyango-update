...
export default function ShortReportPage() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const url =
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/reports/short/` +
      ...
      <div ref={printRef}>
        <table className="min-w-full border-collapse border border-gray-300 dark:border-gray-700 mt-4 text-gray-900 dark:text-white">
          ...
          <tbody>
            {report.length === 0 ? (
              <tr>
                <td colSpan={8} className="text-center px-4 py-4 text-gray-500 dark:text-gray-400">
                  No sales data found for this date range.
                </td>
              </tr>
            ) : (
              <>
                {report.map(({ date, total_sales, total_discount, total_refunds, total_expenses, loans, profit, sales_count }) => (
                  <tr key={date}>
                    <td className="border px-4 py-2">{date}</td>
                    <td className="border px-4 py-2">{Number(total_sales).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(total_discount).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(total_refunds).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(total_expenses).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(loans).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(profit).toLocaleString()}</td>
                ...
                {totals && (
                  <tr className="font-bold bg-gray-200 dark:bg-gray-900">
                    ...
                    <td className="border px-4 py-2">{Number(totals.total_sales).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(totals.total_discount).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(totals.total_refunds).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(totals.total_expenses).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(totals.total_loans).toLocaleString()}</td>
                    <td className="border px-4 py-2">{Number(totals.total_profit).toLocaleString()}</td>

...
export default function SalesPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Sales" subtitle="View and filter sales records" />
      <ContentCard noPadding>
        ...
        {loading && <p className="px-5 py-6 text-gray-600 dark:text-gray-300 sm:px-6">Loading sales…</p>}
        {error && <p className="px-5 py-3 text-error-600 font-semibold sm:px-6">{error}</p>}
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-white/10">
                    {filteredSales.length > 0 ? (
                        ...
                        <React.Fragment key={sale.id}>
                          ...
                          >
                            ...
                            <td className="px-5 py-4 text-red-600 dark:text-red-400">
                              {Number(sale.discount_amount ?? 0) > 0 ? `-${safeTZS(sale.discount_amount)}` : '-'}
                            </td>
                            ...
                            <td className="px-5 py-4 max-w-[150px]">
                              {sale.fulfillment_status === 'checked' ? (
                                <span className="text-xs text-green-700 dark:text-green-400 flex items-center gap-1" title={`Checked by ${sale.checked_by_username ?? '—'}${sale.checked_at ? ` at ${new Date(sale.checked_at).toLocaleString()}` : ''}`}>
                                  <CheckCircle className="h-3.5 w-3.5 shrink-0" />
                                  <span className="truncate">Checked{sale.checked_by_username ? ` by ${sale.checked_by_username}` : ''}</span>
                                </span>
                              ) : (
                            ...
                            <td className="px-5 py-4 space-x-2">
                              <Link
                                href={`/invoice/shop/${sale.id}`}
                                onClick={(e) => e.stopPropagation()}
                                className="inline-flex items-center rounded border border-gray-300 px-3 py-1 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800"
                              >
                                Invoice
                              </Link>
                              {canRefund(sale) ? (
                            ...
                            <tr className="bg-gray-100 dark:bg-white/10">
                              <td colSpan={15} className="px-5 py-3">
                                <div className="space-y-3">
                                  {/* Summary row */}
                                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm pb-3 border-b border-gray-200 dark:border-gray-700">
                                    <div>
                                      <span className="text-gray-500 dark:text-gray-400 block text-xs">Subtotal</span>
                                      <span>{safeTZS(sale.total_amount)}</span>
                                    </div>
                                    <div>
                                      <span className="text-gray-500 dark:text-gray-400 block text-xs">Discount</span>
                                      <span className="text-red-600 dark:text-red-400">
                                        {Number(sale.discount_amount ?? 0) > 0 ? `-${safeTZS(sale.discount_amount)}` : '-'}
                                      </span>
                                    </div>
                                    <div>
                                      <span className="text-gray-500 dark:text-gray-400 block text-xs">Final total</span>
                                      <span className="font-semibold">{safeTZS(sale.final_amount)}</span>
                                    </div>
                                    <div>
                                      <span className="text-gray-500 dark:text-gray-400 block text-xs">Store check</span>
                                      {sale.fulfillment_status === 'checked' ? (
                                        <span className="text-xs text-green-600 dark:text-green-400">
                                          Checked by {sale.checked_by_username ?? '—'}
                                          {sale.checked_at ? ` · ${new Date(sale.checked_at).toLocaleDateString()}` : ''}
                                        </span>
                                      ) : (
                      ...
                      <tr>
                        <td colSpan={15} className="px-5 py-4 text-center text-gray-500 dark:text-gray-400">
